...
const name = p
    .regex(/[_\w][_\d\w]*/)
    .result((x) => ({ kind: 'Name', value: x[0] }));
const null_ = p
    .string('null')
    .result(() => ({ kind: 'NullValue', value: null }));
...
const enum_ = p.wrapResult(p.negativeLookahead('Enum values cannot be true, false or null', p.regex(/true|false|null/), name), (value) => ({ kind: 'EnumValue', value }));
...
const value = p
    .chain(p.oneOf(null_, bool, variable, string_, number_, enum_, list, object), p.optional(ignored))
    .result((v) => v);
...
const operationDefinition = p
    .chain(p.optional(ignored), p.oneOf(p.string('query'), p.string('mutation'), p.string('subscription')), p.optional(p.chain(ignored, name).result((_, name) => name)), p.optional(ignored), p.optional(varDefinitions), directives, selectionSet)
...
exports.parse = parse;
//# sourceMappingURL=graphql-explicit-types.example.js.map